<#include "/_page.ftl"><@pageLayout>
<#if site.categories?? &&  (site.categories?size > 0)>
<#macro subcategory parent level>
<#if (parent.children?size > 0)>
    <ul class="children${level}">
<#list parent.children as child>
	<li class="category"><a href="${root_url}${child.url}">${child.name}</a> (${child.posts?size})
	<@subcategory parent=child level=level+1/></li>
</#list>
    </ul>
</#if>
</#macro>

<div id="blog-archives">
<ul style="padding-left:40px;">
<#list site.categories as category>
	<#if !(category.parent)??>
	<li class="category"><a href="${root_url}${category.url}">${category.name}</a> (${category.posts?size})
	<@subcategory parent=category level=1/></li>
	</#if>
</#list>
  </ul>
 </div>
</#if>
</@pageLayout>